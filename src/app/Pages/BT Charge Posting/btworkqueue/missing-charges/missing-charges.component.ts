...
declare var $: any;
@Component({
  selector: "app-missing-charges",
  templateUrl: "./missing-charges.component.html",
  styleUrls: ["./missing-charges.component.css"],
})
export class MissingChargesComponent implements OnInit, OnDestroy {
  ...
  showReleaseAssignment = false;
  // public isAssignedtask = false;
  // public isbtnAssignedtask = false;
  exportFilename: string = "PendingEncounters";
    ...
    this.subscription.add(
      this.dataService.navSubject.subscribe((data) => {
        if (data != null || data != undefined) {
          ...
          this.showReleaseAssignment = data.viewReleaseAssignment;
  ...
  onReleaseAssignmentClick() {
    try {
      ...
      orderreleasedetails.nstatus = 0; //new change
        ...
        this.coreService.releaseOrderAssignment(orderreleasedetails).subscribe(
          (data) => {
            if (data == 1) {
              ...
              this.writeLog(
                "Pending Encounters: Encounter assignment released successfully.",
                "UPDATE"
              );
              this.OrderSelected = [];
              this.lstFilter = this.dataService.SelectedFilter;
  ...
  onReleaseBtnClick() {
    try {
      this.confirmationTitle = "Release Assignment Confirmation";
      this.confirmationFrom = "releaseassignment";
      this.InputStatusMessage =
        "Do you want to release assignment of selected encounter(s)?";
    } catch (error) {
      this.clsUtility.LogError(error);
    }
  }
