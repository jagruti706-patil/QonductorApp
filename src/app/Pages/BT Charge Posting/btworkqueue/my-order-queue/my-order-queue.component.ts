...
@Component({
  selector: "app-my-order-queue",
  templateUrl: "./my-order-queue.component.html",
  styleUrls: ["./my-order-queue.component.css"],
})
export class MyOrderQueueComponent implements OnInit, OnDestroy {
    ...
    try {
      if (this.OrderItems != null) {
        ...
        if (this.OrderSelected.length == 0 && AllTask == 1) {
          this.clsUtility.showInfo("Select encounter to work");
          return;
        } else {
          if (AllTask == 0) {
